...
lookback    = 25                                                                # Memoria de la red neuranal
...
clm_x       = clm_data.iloc[ lookback : lookback + train_days,  : ][ target_cols ].to_numpy()
...
model.add( Dense( train_days ) )

### Entrenamos el modelo
model.compile( loss='mean_squared_error', optimizer='adam' )
model.fit( train_x, train_y, epochs = 15000, batch_size = train_ccaa, verbose = 1 )
...
prediction = np.resize( prediction.astype( int ), ( prediction.shape[ 2 ] ) )

### Generamos unos gráficos para validación visual
plt.plot( prediction, label = "Predicción" )
plt.plot( clm_data.iloc[ lookback : train_days + lookback, : ][ target_cols ].to_numpy(), label = "Datos experimentales" )
...
for ccaa in ccaa_data:
    exp_data    = ccaa_data[ ccaa ][ target_cols ].to_numpy()[ lookback : train_days + lookback, : ]
    exp_data    = np.resize( exp_data, ( 1, 1, train_days ) )
    
    pred        = model.predict( exp_data, batch_size = 1 )
    pred        = np.resize( pred.astype( int ), ( train_days ) )
    
    exp_data    = np.resize( exp_data, ( train_days ) )
